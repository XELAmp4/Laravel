...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{__('content.title.changePassword')}}</title>
    ...
    <nav class="bg-white border-gray-200 dark:bg-gray-900">
            <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                ...
                <svg class="h-8" viewBox="0 0 286 314" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">{{__('content.title.changePassword')}}</span>
                </a>
                ...
                <div class="relative py-3 sm:max-w-xl sm:mx-auto">
                    ...
                    <div class="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
                        <div class="max-w-md mx-auto">
                            <div>
                                <h1 class="text-2xl font-semibold">{{__('content.title.changePassword')}}</h1>
                            </div>
                            <form action="{{ route('ChangePassController', $idpass) }}" method="post" class="divide-y divide-gray-200">
                            @csrf 
                                <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                                    <div class="relative">
                                        <input autocomplete="off" name="password" type="password" class="peer placeholder-transparent h-10 w-full border-b-2 border-gray-300 text-gray-900 focus:outline-none focus:borer-rose-600 @error('password') is-invalid @enderror" placeholder="www.example.com" required/>
                                    ...
                                    <div class="relative">
                                        <button class="bg-blue-500 text-white rounded-md px-2 py-1">{{__('content.title.changePassword')}}</button>
                                    </div>
                                </div>
                                @if ($errors->any())
                                    @error('password')
